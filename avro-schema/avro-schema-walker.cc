...
void AvroSchemaWalker::StartWalk(const NodePtr node, vector<string>& names,
  ...
  VLOG(2) << "Inside StartWalk. Level: " << level << ". Leaf count: " << node->leaves() << ". Name count: " << node->names() << ". Type: " << node->type();
...
void* AvroSchemaToParquetSchemaConverter::AtNode(const NodePtr& node, vector<string>& names, 
						 int level,
						 void* parent_data) {
  ...
  if (level == 0 && parent_data == nullptr && names.size() == 0) {
    LOG_IF(FATAL, node->type() != avro::AVRO_RECORD) << "Root node was not AVRO_RECORD";
...
ParquetColumn* AvroSchemaToParquetSchemaConverter::AvroNodePtrToParquetColumn(const NodePtr& node,
  ...
  if (node->type() == avro::AVRO_ARRAY) {
    ...
    c = new ParquetColumn(name, parquet::Type::INT32,
			  1, 1,
